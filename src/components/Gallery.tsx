        ...
        <>
            <div className="space-y-6">
                ...
                <div className="bg-container rounded-container p-8">
                    ...
                    {isLoading && !videos &&
                        <p className="text-gray-400">Loading videos...</p>} {/* Show loading only on initial load */}
                    {error &&
                        <p className="text-red-400 flex items-center gap-2"><AlertTriangle size={18}/> Failed to load
                            videos.</p>}

                    {!isLoading && !error && videos && videos.length === 0 && (
                        <p className="text-gray-400">You haven&apos;t generated any videos yet, or they are still
                            processing.</p>
                    )}
                            ...
                            {displayedVideos.map((video) => (
                                ...
                                >
                                    ...
                                    {video.status === 'completed' && video.imageUrl ? (
                                        ...
                                        <div className="relative group aspect-video w-full">
                                            {video.imageUrl && (
                                                <Image src={video.imageUrl} alt="Input Image" fill
                                                       className="object-cover"/>
                                            )}

                                            <Image src="/loading.gif" alt="Loading Image" fill
                                        ...
                                        <div
                                            className="aspect-video flex flex-col items-center justify-center text-red-400 bg-[#1A1A1D] rounded-2xl">
                                            <AlertTriangle size={24} className="text-red-400 mb-2"/>
                                            <span className="text-red-400">Failed</span>
                                            {video.imageUrl && (
                                                <Image src={video.imageUrl} alt="Input Image" width={64} height={64}
                                    ...
                                    {!showViewAll && (
                                        <div
                                            className="p-3 absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                            <p className="text-sm text-white truncate font-semibold"
                                               title={video.prompt}>{video.prompt}</p>
                                        </div>
                                    )}
