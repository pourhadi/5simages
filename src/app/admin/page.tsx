...
export default function AdminPage() {
  ...
  const { data: videos } = useSWR('/api/admin/videos', fetcher);

  const handleSetCredits = async (id: string, current: number) => {
    const input = prompt('Set credits', String(current));
    ...
    try {
      await axios.patch(`/api/admin/users/${id}/credits`, { credits });
      mutateUsers();
    } catch (err) {
      console.error(err);
